...
<?php
    if(isset($_POST["submit"])){
        ...
        $upload_folder = "admin_background/" . $image_name;
        move_uploaded_file($temp_name, $upload_folder);

        $sql = "UPDATE admin SET background = '$image_name' WHERE email = '$email'";
        $result = mysqli_query($conn, $sql);

        if($result){
            echo"<script>
            window.location.href='admin_page.php'; alert('Background Image Updated Successfully');
            </script>";
        }
    }
?>